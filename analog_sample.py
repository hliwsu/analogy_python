...
while t_concurrent < t1:
    ...
    t_total.append(t)
dist_na = np.load(
    "/Users/haoli/OneDrive/Work/Analogy/Python codes/sel_data/error_0.npy")
...
for item in ["ppt", "tmax", "tmin"]:
    locals()[item+'_1895_1950'] = np.load(
        "/Users/haoli/OneDrive/Work/Analogy/Python codes/sel_data/"+item+"_1895_1950_sel.npy")
    locals()[item+'_1951_2014'] = np.load(
        "/Users/haoli/OneDrive/Work/Analogy/Python codes/sel_data/"+item+"_1951_2014_sel.npy")
    locals()[item+'_h'] = np.concatenate((locals()
                                          [item+'_1895_1950'], locals()[item+'_1951_2014']))
    locals()[item+'_yr_mean_p1'] = locals()[item+'_h'].reshape((int(locals()
                                                                    [item+'_h'].shape[0]/12), 12, len(dist_id_sel))).mean(axis=(1))
    yr_p1 = len(locals()[item+'_yr_mean_p1'])+1894
    locals()[item+'_rcp85'] = np.load(
        "/Users/haoli/OneDrive/Work/Analogy/Python codes/sel_data/"+item+"_rcp85_sel.npy")
    locals()[item+'_yr_mean_p2'] = locals()[item+'_rcp85'].reshape((int(locals()
                                                                        [item+'_rcp85'].shape[0]/12), 12, len(dist_id_sel))).mean(axis=(1))
    locals()[item+'_yr_mean_p2'] = locals()[item +
                                            '_yr_mean_p2'][-(2099-yr_p1):, :]
    locals()[item+'_yr_mean'] = np.concatenate((locals()
                                                [item+'_yr_mean_p1'], locals()[item+'_yr_mean_p2']), axis=0)
    # creating A (historical) matrix: 1975-2014
    locals()[item+'_A'] = locals()[item +
                                   '_yr_mean'][(1975-1895):(2015-1895), ].mean(axis=0)
    # yr_A=yr[(1975-1895):(2015-1895)]
    ...
    locals()[item+'_C'] = locals()[item +
                                   '_h'][t_total.index("1895-01"):t_total.index("1995-01"), ]
...
for ref_dist in range(0, len(dist_id_sel)):
    ...
    B_j = B_agg[:, ref_dist]
    C_j = np.transpose(np.concatenate((ppt_C[:, ref_dist], tmax_C[:, ref_dist], tmin_C[:, ref_dist])).reshape(
        3, len(ppt_C)))  # C_j should be T_mon X n(var), T X K in Mohany's paper
...
dist_names_valid = np.load(
    "/Users/haoli/OneDrive/Work/Analogy/Python codes/analog/dist_n_a.npy")
...
for min_ind in range(0, min_dist_val.shape[0]):
    ...
    print("Future", dist_names_valid[min_ind], "is analogous to current",
          dist_names_valid[int(min_dist_ind[min_ind])])
...
# ending datetime.datetime(2020, 6, 4, 21, 48, 4, 888163) i7-6700k (hyperthreading enabled), 32GB, 40 seconds
# starting 2020-06-05 00:57:34.575944 R5-3550H, 16GB, plugged in
# ending  2020-06-05 00:57:34.575944 R5-3550H, 16GB, plugged in, 32 seconds
# E3-1225 V3, 16GB, 32.104797 seconds
# i7-6700k (hyperthreading disabled), 32GB, 25.347667 seconds
# i5-3230m, 8GB, macOS, 101 seconds
# i7-5650u, 8GB, macOS, 80.902989
