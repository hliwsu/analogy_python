...
dist_names_valid=np.load("C:\\Users\\haoli\\OneDrive\\Work\\Analogy\\Python codes\\sel_data\\dist_n_a.npy")
dist_names_all=np.load("C:\\Users\\haoli\\OneDrive\\Work\\Analogy\\Python codes\\sel_data\\dist_n_all.npy")
dist_names_id=np.load("C:\\Users\\haoli\\OneDrive\\Work\\Analogy\\Python codes\\sel_data\\dist_id_valid.npy")

# read off shapefile info and pass the parameters/polygons to the map_plotting function
bmp = Basemap(llcrnrlon=-119,llcrnrlat=22,urcrnrlon=-64,urcrnrlat=49,     projection='lcc',lat_1=33,lat_2=45,lon_0=-95) # define a basemap class that covers the U.S.
shp_info = bmp.readshapefile("D:\\USRD\\S_USARangerDistrict","rld",drawbounds=True,linewidth=2) # load rangeland districts (RLD) shapefile
# set initial parameters
colors={} # init colors in a dict
rld_ids={} # init rangeland district IDs, using DISTRICTOR in the shapefile as the key in a dict
rldnames=[] # init rangeland district names, using DISTRICTNA in the shapefile as the key
for shapedict in bmp.rld_info:
    DISTRICTOR=int(shapedict['DISTRICTOR']) # concurrent RLD ID in this loop
    DISTRICTNA = str(shapedict['DISTRICTNA'])  # concurrent RLD name in this loop
    colors[DISTRICTNA]=(1.0, 0.6166666229167378, 0.0) # assign color values to the concurrent district, in RGBa format, neglect "a" 
    rld_ids[DISTRICTNA]=int(shapedict['DISTRICTOR']) # assign IDs to the concurrent district, in RGBa format, neglect "a" 
    rldnames.append(DISTRICTNA) 
# replace / in RLD names that does not usable in computer programming 
    
'''
Usually, I am very reluctant to use a function in a straight forward algorithm or calculation, but since the plottings are relatively independent, and it would be unnecessary to revise the entire algorithm to fit a plotting in, so I decided to use two functions: map plotting and density plotting. 
'''
# define a map plotingg function 
def map_plotting(ftr, alg): #ftr: future, a value; alg: analogs, can be a list or value
    # determine if it has analogs
    if len(alg)!=0:
        # plot a basemap
        plt.figure(figsize=(100,100))
        bmp.drawcountries(linewidth=4)
        bmp.drawcoastlines(linewidth=4)
        bmp.drawstates(linewidth=3)
        bmp.drawcounties(linewidth=0.25)    
        # pass down the analog list
        x_4=[]
        for x3 in alg:
            # print(dist_names_valid[x3])
            x_4.append(dist_names_valid[x3])
        ax = plt.gca() # get current axes instance
        for nshape,seg in enumerate(bmp.rld):
            if rldnames[nshape] in x_4:
                #print(colors[rldnames[nshape]])
                #print(rldnames[nshape])
                color = "#E5E11D" 
                poly = Polygon(seg,facecolor=color,edgecolor=color)
                ax.add_patch(poly)
            if rldnames[nshape]==dist_names_valid[ftr]:
                color = '#00FF00'
                poly = Polygon(seg,facecolor=color,edgecolor=color)
                ax.add_patch(poly)
            if (rldnames[nshape] not in x_4) and (rldnames[nshape]!=dist_names_valid[ftr]): 
                color = '#DBE7E8'
                poly = Polygon(seg,facecolor=color,edgecolor=color)
                ax.add_patch(poly)
        str_dist_names=dist_names_valid[ftr].replace("/", "-")
        plt.savefig(mdl+"_B"+mdl_c_disp+"_map_"+str_dist_names+".jpg")
        plt.close()
        
# define a density plotting function
# def map_plotting(a, b): # a: historical data, b, future data
...
trunc_eig_val = 0.095 #/ crucial, the larger the truncation value is, the more consistent the analog results will be. 0.1 candidate, 0.25 too , 0.15 too large, 0.11 maybe (ongoing testing)
for mdl in model:
    ...
    for item in ["ppt", "tmax", "tmin"]:
        ...
        locals()[item+"_f_"+mdl]=np.delete(locals()[item+"_f_all_"+mdl],[it for it in range(0,(2015-2006)*12) ],0) # delete overlapping years of data
        # stack historical and future data
        locals()[item+"_all_"+mdl]=np.concatenate((locals()[item+'_h'], locals()[item+"_f_"+mdl]), axis=0) # .shape (2460, 489)
        ...
        locals()[item+"_all_"+mdl]=locals()[item+"_all_"+mdl][t_total.index("1895-10"):t_total.index("2099-10"),]
        ...
        locals()[item+'_sp_'+mdl]=locals()[item+'_seasonal_all_'+mdl][1].mean(axis=1)
        locals()[item+'_sm_'+mdl]=locals()[item+'_seasonal_all_'+mdl][2].mean(axis=1)
        locals()[item+'_fl_'+mdl]=locals()[item+'_seasonal_all_'+mdl][3].mean(axis=1)
        locals()[item+'_wt_'+mdl]=locals()[item+'_seasonal_all_'+mdl][0].mean(axis=1)
    ...
    for B_agg in (locals()[mdl+"_B1"], locals()[mdl+"_B2"], locals()[mdl+"_B3"]):
        ...
        for ref_dist in range(0,len(dist_id_sel)):
            ...
            for int_loop in range(0,len(dist_id_sel)):
                ...
                distances[int_loop,ref_dist]=np.linalg.norm(X_prime[int_loop, 0:C_eigen_val_count]-Y_j_prime[0:C_eigen_val_count])
                print(ref_dist,int_loop,B_ctrl, mdl_c)
        mdl_c_disp=str(B_ctrl+1)
        locals()[mdl+"_distance_vals_B"+mdl_c_disp]= distances
        locals()[mdl+"_distance_pct_B"+mdl_c_disp]=chi.cdf(distances, C_eigen_val_count)
        ...
        for min_ind in range(0,min_dist_val.shape[0]):
            ...
            if analog_ct==0:
                ...
                print(dist_names_valid[int(min_dist_ind[-1])], file=f_c_d_na_min)
