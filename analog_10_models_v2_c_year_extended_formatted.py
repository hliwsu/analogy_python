...
while t_concurrent < t1:
    ...
    t_total.append(t)
dist_na = np.load(
    "C:/Users/haoli/OneDrive/Work/Analogy/Python codes/sel_data/error_0.npy")
dist_id_all = list(range(0, 503))
dist_id_sel = [e for e in dist_id_all if e not in dist_na]
dist_id_sel = list(dist_id_sel)
dist_names_valid = np.load(
    "C:/Users/haoli/OneDrive/Work/Analogy/Python codes/analog/dist_n_a.npy")
dist_names_all = np.load(
    "C:/Users/haoli/OneDrive/Work/Analogy/Python codes/analog/dist_n_all.npy")
dist_names_id = np.load(
    'C:/Users/haoli/OneDrive/Work/Analogy/Python codes/analog/dist_id_valid.npy')
...
f_d = f_n = f_n_ext = []
f = open('results', 'a')
f_f_d_na = open('future_dist_names', 'a')
f_f_d_id = open('future_dist_id', 'a')
f_c_d_na = open('current_dist_names', 'a')
f_c_d_id = open('current_dist_id', 'a')
f_mdl = open('in_models', 'a')
...
for mdl in model:
    ...
    for item in ["ppt", "tmax", "tmin"]:
        ...
        locals()[item+'_h'] = np.concatenate((locals()[item+'_1895_1950'],
                                              locals()[item+'_1951_2014']))  # get the historical data set
        locals()[item+'_yr_mean_p1'] = locals()[item+'_h'].reshape((int(locals()[item+'_h'].shape[0]/12),
                                                                    12, len(dist_id_sel))).mean(axis=(1))  # reshape historical data set according to years
        yr_p1 = len(locals()[item+'_yr_mean_p1'])+1894  # obtain a year index
        # obtain future data
        # load future data (MC2 results series). It can also be obtained using "future_n_p_tma_tmi_v_" index but I decided to save some spaces:)
        locals()[item+"_"+mdl] = np.load(ppt_future[m_control])
        locals()[item+'_yr_mean_p2'] = locals()[item+"_"+mdl].reshape((int(locals()[item+"_"+mdl].shape[0]/12),
                                                                       12, len(dist_id_sel))).mean(axis=(1))  # reshape future data set according to years
        # keeping PRISM data in the duplicate years while abondaning future data in conflict years
        locals()[item+'_yr_mean_p2'] = locals()[item +
                                                '_yr_mean_p2'][-(2099-yr_p1):, :]
        locals()[item+'_yr_mean'] = np.concatenate((locals()[item+'_yr_mean_p1'], locals()
                                                    [item+'_yr_mean_p2']), axis=0)  # generate full year datasets, 1895-2099, 205 years total
        # creating sub A matrix (part 1/2: ppt, tmax, tmin, historical matrix, 1975-2015)
        locals()[item+'_A'] = locals()[item+'_yr_mean'][(1975-1895):(2015-1895),
                                                        ].mean(axis=0)  # yr_A=yr[(1975-1895):(2015-1895)] ## check years
        locals()[item+'_'+mdl+'_A_p1'] = np.append(A, locals()[item+'_A'])
        # creating sub B matrix (part 1/2: ppt, tmax, tmin, future matrix, 2050-2099 )
        locals()[item+'_B'] = locals()[item +
                                       '_yr_mean'][2050-2099+1:, ].mean(axis=0)
        ...
        # check results, C_sub matrices in here should be the same for each item
        locals()[item+'_'+mdl+'_C_p1'] = locals()[item +
                                                  '_yr_mean'][0:(2015-1895), ]
        locals()['A_'+mdl] = np.append(locals()
                                       ['A_'+mdl], locals()[item+'_'+mdl+'_A_p1'])
        locals()['B_'+mdl] = np.append(locals()
                                       ['B_'+mdl], locals()[item+'_'+mdl+'_B_p1'])
        i = i+1
    ...
    # np.append is used as above to avoid any unnecessary but potential dimension issues, reshape applied in the following
    locals()['A_'+mdl] = np.append(locals()['A_'+mdl], npp_a_p2)
    ...
    npp_b_p2 = np.load(
        locals()["future_n_p_tma_tmi_v_"+model_file][0])[2050-2099+1:, ].mean(axis=0)
    locals()['B_'+mdl] = np.append(locals()['B_'+mdl], npp_b_p2)
    vtype_b_p2 = np.load(
        locals()["future_n_p_tma_tmi_v_"+model_file][4])[2050-2099+1:, ].mean(axis=0)
    locals()['B_'+mdl] = np.append(locals()['B_'+mdl], vtype_b_p2)
    locals()['B_'+mdl+'_agg'] = locals()['B_'+mdl].reshape(5, len(dist_id_sel))
    # the initial value of m is outside of the loop, so 10 loops lead to m=10
    ...
    for ref_dist in range(0, len(dist_id_sel)):
        ...
        B_j = B_agg[:, ref_dist]
        C_j = np.transpose(np.concatenate((locals()['ppt_'+mdl+'_C_p1'][:, ref_dist], locals()['tmax_'+mdl+'_C_p1'][:, ref_dist], locals()['tmin_'+mdl+'_C_p1'][:, ref_dist], locals()[
                           "npp_"+mdl+"_C_p2"][:, ref_dist], locals()["vtype_"+mdl+"_C_p2"][:, ref_dist])).reshape(5, len(locals()['ppt_'+mdl+'_C_p1'])))  # C_j should be T_mon X n(var), T X K in Mohany's paper
        C_j_sd = C_j.std(axis=0)
        if C_j_sd[4] == 0:
            locals()[mdl+"_std_zero"] = np.append(locals()
                                                  [mdl+"_std_zero"], ref_dist)
            continue
    ...
    for min_ind in range(0, min_dist_val.shape[0]):
        locals()[mdl+'min_dist_ind'] = np.append(locals()[mdl+'min_dist_ind'],
                                                 np.where(distances[:, min_ind] == np.amin(min_dist_val[min_ind])))
        print("Future", dist_names_valid[min_ind], "is analogous to current", dist_names_valid[int(
            locals()[mdl+'min_dist_ind'][min_ind])], file=f)
        print(dist_names_valid[min_ind], file=f_f_d_na)
        print(min_ind, file=f_f_d_id)
        print(dist_names_valid[int(
            locals()[mdl+'min_dist_ind'][min_ind])], file=f_c_d_na)
        print(int(locals()[mdl+'min_dist_ind'][min_ind]), file=f_c_d_id)
...
# i7-6700k, 32GB, hyperthreading disabled, 0:07:48.540414
