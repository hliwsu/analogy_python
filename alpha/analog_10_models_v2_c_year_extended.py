...
list1 =  [24, 219,270,400,418,423,430,435,436,444,474]
...
A=[];B=[];C=[]
distances=np.zeros(shape=(478,478)) # use precalculated values
...
for mdl in model:
    ...
    for item in ["ppt", "tmax", "tmin"]:
        ...
        locals()[item+'_1895_1950']=np.delete(locals()[item+'_1895_1950'],list1,1)
        # load historical data (PRISM series)
        locals()[item+'_1951_2014']=np.load(prism_2_hist[i])
        # load historical data (PRISM series)
        locals()[item+'_1951_2014']=np.delete(locals()[item+'_1951_2014'],list1,1)
        locals()[item+'_h']=np.concatenate((locals()[item+'_1895_1950'],locals()[item+'_1951_2014'])) # get the historical data set
        locals()[item+'_yr_mean_p1']=locals()[item+'_h'].reshape((int(locals()[item+'_h'].shape[0]/12),12,478)).mean(axis=(1)) # reshape historical data set according to years
        ...
        locals()[item+"_"+mdl]=np.delete(locals()[item+"_"+mdl],list1,1)
        
        locals()[item+'_yr_mean_p2']=locals()[item+"_"+mdl].reshape((int(locals()[item+"_"+mdl].shape[0]/12),12,478)).mean(axis=(1)) # reshape future data set according to years
    ...
    npp_a_p2=np.delete(npp_a_p2,list1,0)
    ...
    vtype_a_p2=np.delete(vtype_a_p2,list1,0)
    locals()['A_'+mdl]=np.append(locals()['A_'+mdl],vtype_a_p2)
    locals()['A_'+mdl+'_agg']=locals()['A_'+mdl].reshape(5, 478)
    ...
    npp_b_p2=np.delete(npp_b_p2,list1,0)
    ...
    vtype_b_p2=np.delete(vtype_b_p2,list1,0)
    locals()['B_'+mdl]=np.append(locals()['B_'+mdl],vtype_b_p2)
    locals()['B_'+mdl+'_agg']=locals()['B_'+mdl].reshape(5, 478)
    ...
    locals()["npp_"+mdl+"_C_p2"]=np.delete(locals()["npp_"+mdl+"_C_p2"],list1,1)
    locals()["vtype_"+mdl+"_C_p2"]=np.load(mc2_hist[1])[0:(2015-1895),]
    locals()["vtype_"+mdl+"_C_p2"]=np.delete(locals()["vtype_"+mdl+"_C_p2"],list1,1)
    ...
    for ref_dist in range(0,478):
        ...
        #rb_a = list(range(0, len(dist_id_sel)))
        #list1 =  [24, 219,270,400,418,423,430,435,436,444,474]
        #rb_a=[x for x in rb_a if x not in list1]
        for int_loop in range(0,478):
    ...
    #for del_ind in locals()[mdl+"_std_zero"]:
        #min_dist_val=np.delete(min_dist_val,[del_ind,])
        #dist_names_valid=np.delete(dist_names_valid,[del_ind,])
        #distances_temp=np.delete(distances,[del_ind,])
    ...
    for min_ind in range(0,min_dist_val.shape[0]):    
        ...
        print(dist_names_valid[int(locals()[mdl+'min_dist_ind'][min_ind])], file=f_c_d_na)
